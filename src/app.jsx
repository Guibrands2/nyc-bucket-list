...
const INITIAL_PLACES = [
  ...
  { id:"t013", bestTime:"Apos as 19h, experiencia noturna incrivel", category:"Observatorios", name:"SUMMIT One Vanderbilt", nameEN:"SUMMIT One Vanderbilt", emoji:"🔮", desc:"Instalacoes de arte com espelhos e vidro, vistas deslumbrantes. Abre ate meia-noite.", descEN:"Art installations with mirrors and glass, stunning views. Open until midnight.", price:"$$$", lat:40.7527, lng:-73.9772, time:"2h", link:"https://summitov.com", petFriendly:false, publicBathroom:true, season:"sempre", rep:"O mais impressionante dos observatorios.", repEN:"The most impressive of the observatories." },
  { id:"t014", bestTime:"Por do sol, chegue 30min antes das 17h", category:"Observatorios", name:"Top of the Rock", nameEN:"Top of the Rock", emoji:"🏙️", desc:"No Rockefeller Center, com a view classica com o Empire State no meio da foto.", descEN:"At Rockefeller Center, with the classic view of the Empire State in the photo.", price:"$$$", lat:40.7593, lng:-73.9787, time:"1h", link:"https://www.topoftherocknyc.com", petFriendly:false, publicBathroom:true, season:"sempre", rep:"Va no por do sol para ver a cidade acender.", repEN:"Go at sunset to see the city light up." },
  { id:"t015", bestTime:"Entardecer, por volta das 17h para ver a cidade acender", category:"Observatorios", name:"Empire State Building", nameEN:"Empire State Building", emoji:"🌃", desc:"O icone absoluto de Nova York. Abre ate 11:30pm, otimo pra ir ao anoitecer.", descEN:"The absolute icon of New York. Open until 11:30pm, great for sunset.", price:"$$$", lat:40.7484, lng:-73.9857, time:"2h", link:"https://www.esbnyc.com", petFriendly:false, publicBathroom:true, season:"sempre", rep:"O classico dos classicos. Reserve online.", repEN:"The classic of classics. Book online." },
...
function SeasonalBanner({ places, entries, onSelect }) {
  ...
  const [bg,border,text] = SEASON_COLORS[currentSeason]||["#1a1a22","#2a2a38","#f0eeff"];
  ...
  return (
    <div onClick={()=>onSelect(pick)} style={{ background:"linear-gradient(135deg,"+bg+",#0f0f13)",border:"1px solid "+border,borderRadius:14,padding:"12px 16px",marginBottom:10,cursor:"pointer",position:"relative",overflow:"hidden" }}>
      ...
      <div style={{ display:"flex",gap:12,alignItems:"center" }}>
        ...
        <div>
          <div style={{ fontSize:14,fontWeight:700,color:"#f0eeff" }}>{isEN&&pick.nameEN?pick.nameEN:pick.name}</div>
          <div style={{ fontSize:11,color:"#9090b0",marginTop:2 }}>{catLabel(pick.category)} · {PRICE_EMOJI[pick.price]||"?"} · {pick.time}</div>
        </div>
