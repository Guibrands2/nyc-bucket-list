...
const T = {
  ...
  createCuradoria: isEN ? "+ Create new list" : "+ Criar nova lista",
  curadoriaEmpty: isEN ? "Create lists to organize your favorites!" : "Crie listas para organizar seus favoritos!",
  curadoriaEx: isEN ? "Ex: Weekend with friends, Rainy day, Romantic NYC" : "Ex: Para ir com amigos, Dia de chuva, NYC romantico",
  curadoriaName: isEN ? "List name..." : "Nome da lista...",
  ...
  myCuradorias: isEN ? "← My lists" : "← Minhas listas",
...
const INITIAL_PLACES = [
  ...
  { id:"t036", category:"Comida", name:"Joe's Pizza", nameEN:"Joe's Pizza", emoji:"🍕", desc:"A fatia de pizza mais classica de NY desde 1975.", descEN:"The most classic NY pizza slice since 1975.", price:"$", lat:40.7306, lng:-74.0022, time:"30min", link:"https://www.joespizzanyc.com", petFriendly:false, publicBathroom:false, season:"sempre", rep:"A pizza perfeita de NYC.", repEN:"The perfect NYC pizza.", hours:"Diario 10h-0h" },
  { id:"t037", category:"Comida", name:"Di Fara Pizza", nameEN:"Di Fara Pizza", emoji:"🍕", desc:"A pizza mais famosa de Brooklyn, feita a mao pelo mesmo dono ha decadas.", descEN:"Brooklyn's most famous pizza, handmade by the same owner for decades.", price:"$", lat:40.6249, lng:-73.9612, time:"1h", link:"https://www.difara.com", petFriendly:false, publicBathroom:false, season:"sempre", rep:"Peregrinacao obrigatoria para fas de pizza.", repEN:"Mandatory pilgrimage for pizza lovers." },
  { id:"t038", category:"Comida", name:"Levain Bakery", nameEN:"Levain Bakery", emoji:"🍪", desc:"O cookie de chocolate mais famoso de NY. Enorme, cremoso e quente.", descEN:"NY's most famous chocolate cookie. Enormous, creamy and warm.", price:"$", lat:40.7812, lng:-73.9803, time:"30min", link:"https://levainbakery.com", petFriendly:false, publicBathroom:false, season:"sempre", rep:"O cookie mais famoso de NYC.", repEN:"NYC's most famous cookie.", hours:"Diario 8h-21h" },
  ...
  { id:"s002", category:"Bairros", name:"Roosevelt Island", nameEN:"Roosevelt Island", emoji:"🌉", desc:"Ilhinha no East River com tramway iconico saindo da 2nd Ave.", descEN:"Small island in the East River with an iconic tramway from 2nd Ave.", price:"gratis", lat:40.7614, lng:-73.9506, time:"2h", link:"https://rioc.ny.gov", petFriendly:true, publicBathroom:false, season:"sempre", rep:"Muito subestimado. O tramway e uma experiencia unica.", repEN:"Very underrated. The tramway is a unique experience." },
  ...
  { id:"s021", category:"Monumentos", name:"Staten Island Ferry", nameEN:"Staten Island Ferry", emoji:"⛴️", desc:"Balsa gratuita com vista frontal da Estatua da Liberdade.", descEN:"Free ferry with a front-row view of the Statue of Liberty.", price:"gratis", lat:40.7007, lng:-74.0132, time:"1h", petFriendly:true, publicBathroom:true, season:"sempre", rep:"A melhor vista gratuita de NYC.", repEN:"The best free view in NYC." },
  ...
  try {
    ...
    if (view==="create") return (
      <div style={{ padding:"0 16px 80px" }}>
        ...
        <div style={{ fontSize:16,fontWeight:700,color:"#1a1a1a",marginBottom:16 }}>{isEN?"New list":"Nova lista"}</div>
    ...
    return <div style={{ padding:"40px 16px", textAlign:"center", color:"#a0a0b0" }}><div style={{ fontSize:32, marginBottom:8 }}>📋</div><div style={{ fontSize:13, color:"#ff2d55", marginBottom:12 }}>{isEN?"Error loading lists":"Erro ao carregar listas"}</div><button onClick={()=>{setError(null);setView("home");}} style={{ padding:"10px 20px", background:"#ff2d55", border:"none", borderRadius:10, color:"#fff", cursor:"pointer" }}>Reload</button></div>;
  }
}
...
function AddModal({ onClose, onAdd, addToast }) {
  ...
  const [addSpentM,setAddSpentM]=useState("");
  const [addStarsM,setAddStarsM]=useState(0);
  const handle=async()=>{
  ...
  return <div style={{ position:"fixed",inset:0,background:"#000000f0",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={onClose}><div className="modal" style={{ background:"#ffffff",borderTop:"1px solid #e8e6e0",borderRadius:"20px 20px 0 0",padding:"20px 16px 48px",maxWidth:560,width:"100%",maxHeight:"85vh",overflowY:"auto" }} onClick={ev=>ev.stopPropagation()}>
    ...
    {/* Date + who + stars + spent — only when "fui" */}
    {status==="fui"&&<div style={{ background:"#f5f3ee80",border:"1px solid #e8e6e0",borderRadius:10,padding:"12px",marginBottom:14 }}>
      ...
      <div style={{ display:"flex",gap:6,marginBottom:10 }}>{WHO_OPTIONS.map(w=><button key={w} onClick={()=>setWho(w)} style={{ flex:1,padding:"8px 4px",borderRadius:8,background:who===w?"#007aff20":"#f5f3ee",border:"1px solid "+(who===w?"#007aff":"#e8e6e0"),color:who===w?"#007aff":"#8a8a9a",fontSize:12,cursor:"pointer" }}>{WHO_EMOJI[w]} {WHO_LABELS[w]}</button>)}</div>
      <div style={{ fontSize:10,color:"#a0a0b0",letterSpacing:"0.1em",marginBottom:6 }}>{isEN?"RATING":"AVALIACAO"}</div>
      <div style={{ display:"flex",gap:4,marginBottom:10 }}>{[1,2,3,4,5].map(n=><span key={n} onClick={()=>setAddStarsM(addStarsM===n?0:n)} style={{ fontSize:26,cursor:"pointer",color:n<=addStarsM?"#ff9f0a":"#a0a0b0",transition:"color 0.1s" }}>★</span>)}</div>
      <div style={{ fontSize:10,color:"#a0a0b0",letterSpacing:"0.1em",marginBottom:6 }}>{isEN?"SPENT":"QUANTO GASTOU"}</div>
      <div style={{ display:"flex",alignItems:"center",gap:8,background:"#ffffff",border:"1px solid #e8e6e0",borderRadius:8,padding:"8px 10px" }}>
        <span style={{ fontSize:15 }}>💸</span>
        <span style={{ fontSize:14,color:"#a0a0b0",fontWeight:600 }}>$</span>
        <input type="number" min="0" step="0.01" value={addSpentM} onChange={ev=>setAddSpentM(ev.target.value)} placeholder="0.00" style={{ flex:1,background:"none",border:"none",color:"#1a1a1a",fontSize:14,fontWeight:600 }}/>
        {addSpentM&&<span style={{ fontSize:11,color:"#30d158" }}>${parseFloat(addSpentM)||0} USD</span>}
      </div>
    </div>}
...
function SurpresaModal({ places, entries, weather, onClose, addToast, onSaveLists, lists }) {
  ...
  if(step==="loading") return (
    <div style={{ position:"fixed",inset:0,background:"#000000f0",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16 }}>
      <div style={{ textAlign:"center" }}>
        <div className="pulsing" style={{ fontSize:52,marginBottom:16 }}>🎲</div>
        <div style={{ fontSize:15,color:"#ffffff",fontWeight:600,marginBottom:6 }}>{isEN?"Building your surprise day...":"Montando seu dia surpresa..."}</div>
        <div style={{ fontSize:12,color:"#ffffff80" }}>{isEN?"Claude is thinking...":"Claude esta pensando..."}</div>
      </div>
      <button onClick={()=>{setStep("form");setLoading(false);}} style={{ marginTop:16,background:"#ffffff20",border:"1px solid #ffffff30",borderRadius:20,padding:"8px 20px",color:"#ffffff",fontSize:13,cursor:"pointer" }}>{isEN?"Cancel":"Cancelar"}</button>
    </div>
  );
    ...
    <div style={{ position:"relative", marginBottom:10, overflow:"hidden", borderRadius:14 }}>
      ...
      >
        <div style={{ position:"relative", flexShrink:0 }}>
          {fp ? <img src={fp} alt="" style={{ width:60,height:60,borderRadius:12,objectFit:"cover" }}/>
              : <div style={{ width:60,height:60,borderRadius:12,background:meta.color+(isBeen?"20":"12"),display:"flex",alignItems:"center",justifyContent:"center",fontSize:28 }}>{place.emoji}</div>}
          {isBeen&&<div style={{ position:"absolute",bottom:-3,right:-3,width:18,height:18,borderRadius:"50%",background:meta.color,border:"2px solid #ffffff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff",fontWeight:700 }}>✓</div>}
          {isWant&&!isBeen&&<div style={{ position:"absolute",bottom:-3,right:-3,width:18,height:18,borderRadius:"50%",background:"#007aff",border:"2px solid #ffffff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff" }}>♥</div>}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:6, marginBottom:5 }}>
            <div style={{ fontSize:16,fontWeight:700,color:"#1a1a1a",lineHeight:1.2,flex:1,letterSpacing:"-0.01em" }}>{displayName}</div>
            <div style={{ display:"flex",gap:3,flexShrink:0,alignItems:"center",marginTop:2 }}>
              {entry?.thumb==="up"&&<span style={{ fontSize:11 }}>👍</span>}
              {entry?.stars>0&&<span style={{ fontSize:11,color:"#ff9f0a",letterSpacing:-1 }}>{"★".repeat(entry.stars)}</span>}
              {pc>0&&<span style={{ fontSize:10,color:"#a0a0b0" }}>📷{pc}</span>}
            </div>
          </div>
          <div style={{ display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom: (entry?.note||entry?.date)?4:0 }}>
            <span style={{ fontSize:11,color:meta.color,background:meta.color+"15",borderRadius:20,padding:"2px 9px",fontWeight:600,letterSpacing:"0.02em" }}>{catLabel(place.category)}</span>
            {displayPrice&&<span style={{ fontSize:11,color:"#a0a0b0",fontWeight:500 }}>{PRICE_EMOJI[displayPrice]}</span>}
            {place.time&&<span style={{ fontSize:11,color:"#c0bdb8" }}>· {place.time}</span>}
            {needsRes&&<span style={{ fontSize:10,color:"#ff9f0a" }}>📋</span>}
            {isPet&&<span style={{ fontSize:10 }}>🐾</span>}
          </div>
          {entry?.note&&<div style={{ fontSize:12,color:"#8a8a9a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontStyle:"italic" }}>"{entry.note}"</div>}
          {isBeen&&entry?.date&&<div style={{ fontSize:11,color:meta.color,fontWeight:500,marginTop:1 }}>✓ {new Date(entry.date+"T12:00:00").toLocaleDateString(isEN?"en-US":"pt-BR",{month:"short",day:"numeric",year:"numeric"})}</div>}
        </div>
      </div>
      {!isBeen&&swipeDx===0&&<button onClick={ev=>{ev.stopPropagation();onCheckIn(place);}} style={{ position:"absolute",bottom:12,right:12,background:"#30d15812",border:"1px solid #30d15830",borderRadius:20,padding:"4px 10px",color:"#30d158",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:600 }}>✓ check-in</button>}
    </div>
  );
...
function AIAddModal({ onClose, onSavePlace, onSaveEvent, addToast }) {
  ...
  const [addStars, setAddStars] = useState(0);
    ...
    <div style={{ position:"fixed",inset:0,background:"#000000f0",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center" }}>
      <div className="modal" style={{ background:"#ffffff",borderTop:"1px solid #e8e6e0",borderRadius:"20px 20px 0 0",padding:"20px 16px 48px",maxWidth:560,width:"100%",maxHeight:"92vh",overflowY:"auto" }}>
        ...
        {addStatus==="fui"&&<div style={{ background:"#f5f3ee80",border:"1px solid #e8e6e0",borderRadius:10,padding:"12px",marginBottom:14 }}>
          ...
          <div style={{ fontSize:10,color:"#a0a0b0",letterSpacing:"0.1em",marginBottom:6 }}>{isEN?"WHO WENT?":"QUEM FOI?"}</div>
          <div style={{ display:"flex",gap:6,marginBottom:10 }}>{WHO_OPTIONS.map(w=><button key={w} onClick={()=>setAddWho(w)} style={{ flex:1,padding:"8px 4px",borderRadius:8,background:addWho===w?"#007aff20":"#f5f3ee",border:"1px solid "+(addWho===w?"#007aff":"#e8e6e0"),color:addWho===w?"#007aff":"#8a8a9a",fontSize:12,cursor:"pointer" }}>{WHO_EMOJI[w]} {WHO_LABELS[w]}</button>)}</div>
          <div style={{ fontSize:10,color:"#a0a0b0",letterSpacing:"0.1em",marginBottom:6 }}>{isEN?"RATING":"AVALIACAO"}</div>
          <div style={{ display:"flex",gap:4,marginBottom:10 }}>{[1,2,3,4,5].map(n=><span key={n} onClick={()=>setAddStars(addStars===n?0:n)} style={{ fontSize:26,cursor:"pointer",color:n<=addStars?"#ff9f0a":"#a0a0b0",transition:"color 0.1s" }}>★</span>)}</div>
          <div style={{ fontSize:10,color:"#a0a0b0",letterSpacing:"0.1em",marginBottom:6 }}>{isEN?"SPENT":"QUANTO GASTOU"}</div>
          <div style={{ display:"flex",alignItems:"center",gap:8,background:"#ffffff",border:"1px solid #e8e6e0",borderRadius:8,padding:"8px 10px" }}>
            <span style={{ fontSize:15 }}>💸</span>
            <span style={{ fontSize:14,color:"#a0a0b0",fontWeight:600 }}>$</span>
            <input type="number" min="0" step="0.01" value={addSpent} onChange={e=>setAddSpent(e.target.value)} placeholder="0.00" style={{ flex:1,background:"none",border:"none",color:"#1a1a1a",fontSize:14,fontWeight:600 }}/>
            {addSpent&&<span style={{ fontSize:11,color:"#30d158" }}>${parseFloat(addSpent)||0} USD</span>}
          </div>
        </div>}
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#f5f3ee",color:"#1a1a1a",fontFamily:"'Inter',system-ui,sans-serif",paddingBottom:80 }}>
      <div style={{ maxWidth:600,margin:"0 auto" }}>
        ...
        {/* Explorar Tab */}
        {tab==="explorar"&&<div style={{ padding:"12px 16px 140px" }}>
          {/* Quick moods */}
          <div style={{ display:"flex",gap:7,overflowX:"auto",scrollbarWidth:"none",paddingBottom:12,marginBottom:2 }}>
          ...
          {placeOfDay&&!search&&activeFilter==="todos"&&activeCategory==="Todos"&&!filterRegion&&(
            <div onClick={()=>openModal(placeOfDay)} style={{ background:"linear-gradient(135deg,#fff5f7,#fff0f8)",border:"1px solid #ffccd5",borderRadius:18,padding:"0",marginBottom:12,cursor:"pointer",position:"relative",overflow:"hidden",boxShadow:"0 2px 12px #ff2d5510" }}>
              <div style={{ height:3,background:"linear-gradient(90deg,#ff2d55,#ff6b35,#ff9f0a)" }}/>
              <div style={{ padding:"14px 16px 16px" }}>
                <div style={{ fontSize:10,color:"#ff2d55",letterSpacing:"0.15em",marginBottom:10,fontWeight:700 }}>{T.placeOfDay}</div>
                <div style={{ display:"flex",gap:16,alignItems:"center" }}>
                  <div style={{ width:72,height:72,borderRadius:16,background:"linear-gradient(135deg,#ff2d5515,#ff6b3510)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:38,flexShrink:0 }}>{placeOfDay.emoji}</div>
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ fontSize:18,fontWeight:800,color:"#1a1a1a",letterSpacing:"-0.02em",lineHeight:1.2,marginBottom:4 }}>{isEN&&placeOfDay.nameEN?placeOfDay.nameEN:placeOfDay.name}</div>
                    <div style={{ display:"flex",gap:6,alignItems:"center",marginBottom:placeOfDay.bestTime?4:0 }}>
                      <span style={{ fontSize:11,color:CAT_META[placeOfDay.category]?.color||"#ff2d55",background:(CAT_META[placeOfDay.category]?.color||"#ff2d55")+"15",borderRadius:20,padding:"2px 9px",fontWeight:600 }}>{catLabel(placeOfDay.category)}</span>
                      <span style={{ fontSize:12,color:"#a0a0b0" }}>{PRICE_EMOJI[placeOfDay.price]||"?"} · {placeOfDay.time||"?"}</span>
                    </div>
                    {placeOfDay.bestTime&&<div style={{ fontSize:11,color:"#ff6b35",fontWeight:500 }}>⏰ {placeOfDay.bestTime}</div>}
                  </div>
                </div>
                {(isEN&&placeOfDay.descEN||placeOfDay.desc)&&<div style={{ fontSize:12,color:"#8a8a9a",marginTop:10,lineHeight:1.5,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical" }}>{isEN&&placeOfDay.descEN?placeOfDay.descEN:placeOfDay.desc}</div>}
                <div style={{ marginTop:10,fontSize:12,color:"#ff2d55",fontWeight:600 }}>{isEN?"See details →":"Ver detalhes →"}</div>
              </div>
            </div>
          )}

          {filteredPlaces.length>0&&<div style={{ fontSize:11,color:"#a0a0b0",marginBottom:8,fontWeight:500 }}>{filteredPlaces.length} {isEN?"places":"lugares"}{hasAnyFilter?" "+isEN?"filtered":"filtrados":""}</div>}
      ...
      {showListas&&<div style={{ position:"fixed",inset:0,background:"#000000c0",zIndex:200 }} onClick={()=>setShowListas(false)}>
        <div onClick={e=>e.stopPropagation()} style={{ position:"absolute",bottom:0,left:0,right:0,background:"#f5f3ee",borderRadius:"20px 20px 0 0",padding:"0 0 40px",maxWidth:600,margin:"0 auto",maxHeight:"88vh",display:"flex",flexDirection:"column" }}>
          <div style={{ padding:"16px 20px 0" }}>
            <div style={{ width:36,height:3,background:"#e8e6e0",borderRadius:2,margin:"0 auto 16px" }}/>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4 }}>
              <div style={{ fontSize:17,fontWeight:700,color:"#1a1a1a" }}>🔖 {isEN?"My Lists":"Minhas Listas"}</div>
              <button onClick={()=>setShowListas(false)} style={{ background:"#f0eeeb",border:"none",borderRadius:8,width:32,height:32,color:"#8a8a9a",cursor:"pointer",fontSize:16 }}>×</button>
            </div>
          </div>
          <div style={{ overflowY:"auto",flex:1 }}>
            <CuradoriaTab places={visiblePlaces} lists={lists} onSaveLists={saveLists} onSelectPlace={p=>{setShowListas(false);openModal(p);}}/>
          </div>
        </div>
      </div>}
